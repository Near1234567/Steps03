...
const Home: React.FC<{ setActiveSection: React.Dispatch<React.SetStateAction<string>>, activeSection: string }> = ({ setActiveSection, activeSection }) => {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    // Using mailto logic similar to price inquiry
    window.location.href = `mailto:${data.contact.inquiryEmail}?subject=${subject}&body=${body}`;
    
    setTimeout(() => {
      setFormStatus('sent');
      setFormData({ name: '', email: '', subject: t('contact.form.subjects.artwork', { defaultValue: 'Information Œuvre' }), message: '' });
              ...
              <div className="relative">
                {formStatus === 'sent' ? (
                  ...
                  <form 
                    ...
                    className={`space-y-6 transition-opacity duration-500 ${formStatus === 'sending' ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}
                  >
