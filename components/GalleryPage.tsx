...
const GalleryPage: React.FC = () => {
  ...
  const selectedArt = currentIndex !== null ? filteredArtworks[currentIndex] : null;

  const handleNext = () => {
    setDirection(1);
    setCurrentIndex((prev) => (prev !== null ? (prev + 1) % filteredArtworks.length : null));
    setShowMagnifier(false);
  };

  const handlePrev = () => {
    setDirection(-1);
    setCurrentIndex((prev) => (prev !== null ? (prev - 1 + filteredArtworks.length) % filteredArtworks.length : null));
    setShowMagnifier(false);
  };
      ...
      <AnimatePresence>
        {currentIndex !== null && selectedArt && (
          ...
          >
            ...
            {/* Navigation Buttons for Lightbox Centered Bottom */}
            <div className="fixed bottom-8 md:bottom-12 left-1/2 -translate-x-1/2 flex items-center space-x-12 z-[2100]">
              <button 
                onClick={(e) => { e.stopPropagation(); handlePrev(); }}
                className="w-14 h-14 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center text-white transition-all backdrop-blur-sm hover:scale-110"
            ...
            <div className="container mx-auto max-w-7xl perspective-2000 py-20 lg:py-0">
               <AnimatePresence mode="wait" custom={direction}>
                  <motion.div 
                    ...
                    className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center preserve-3d"
                  >
                    <motion.div
                    ...
                    >
                      {/* Spine shadow for magazine look */}
